...
class HermesProduct:
    ...
    def get_product_content(self):
        try:
            response = requests.get(f"https://www.hermes.com/{self.country}/{self.language}/category/women/bags-and-small-leather-goods/bags-and-clutches/#|")
    ...
    def send_embed(self, embed):
        try:
            response = requests.post(self.webhook, data = json.dumps({"embeds": embed}), headers = {"Content-Type": "application/json"})
